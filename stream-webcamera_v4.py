...
GPIO.setup(LED_BLINK_QR, GPIO.OUT)
...
def open_camera():
    ...
    GPIO.output(LED_CAMERA, GPIO.HIGH)
...
def close_camera():
    ...
    if camera is not None:
        camera.release()
        camera = None
        GPIO.output(LED_CAMERA, GPIO.LOW)

def handle_find_QR():
    GPIO.output(LED_BLINK_QR, GPIO.HIGH)
    GPIO.output(BUZZER, GPIO.HIGH)
    time.sleep(0.5)
    GPIO.output(LED_BLINK_QR, GPIO.LOW)
